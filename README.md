...
**내 AI가 사고 치기 전에 막는 관문.** 코딩만이 아니라 **영상 편집·배포·데이터**까지. 남이 이미 당한 사고 28건으로 만든 오답노트.

세 층으로 막습니다 — **규칙**(AI가 읽음) · **검사**(내 프로젝트를 훑음) · **관문**(그 자리에서 차단).

> **우리도 걸립니다 — 그리고 고치는 중입니다.** `uv run check.py .` 를 이 저장소에
> 돌리면 E26(덩어리가 너무 큼)이 나옵니다. 495줄짜리 검사 함수 하나를 `selftest.py`
> 로 떼어내 네 덩어리(118·39·148·188줄)로 쪼갰습니다. 남은 것은 `vibe.py`(1,516줄)와
> 동결한 `build.py`·`stack.yaml` 입니다.
> 우리가 안 걸리게 기준을 맞추면 그 기준은 아무도 안 믿습니다.

**「초록불 보고서」** — 통과·성공·완료라는 초록불이 실제로 얼마나 자주 거짓말하는지
세는 정기 보고서입니다. 첫 판은 [보고서/](보고서/) 에 있고 **표본은 1명**입니다.
그대로 적었습니다. 숫자는 `uv run vibe.py 보고서` 가 만들어냅니다 —
손으로 적으면 어긋납니다(P4). 부풀리면 selftest 가 터집니다.
...
**실제로 당한 사고를 AI 에게 알려주고, 그 일이 벌어지려는 순간에 막습니다.**

| 파일 | 무엇 |
|---|---|
| `ERRORS.md` · `vibe.yaml` | **사고 기록 23건.** 이 저장소의 진짜 자산입니다 |
| `guard.py` | **관문** — AI 가 사고를 치기 직전에 막습니다 |
| `check.py` | **검사** — 내 프로젝트에서 그 사고를 찾습니다 |
| `vibe.py` | 놓고 · 갱신하고 · 되돌리고 · 재는 것 |

**아무것도 설치하지 않습니다.** 도구를 안 깔아도 됩니다.

<details>
<summary><b>예전에 만든 것 — 도구 카탈로그 (동결)</b></summary>

`stack.yaml`(도구 31개) · `build.py` · `app.py` 는 2026-08-17 에 만든 **AI 자동화 도구
카탈로그**입니다. 지금은 **더 개발하지 않습니다** — 그 시장에는 이미 스킬 2,810개와
패키지 매니저가 있습니다. 남겨 두는 이유는 지우면 그때 만든 것이 사라지기 때문입니다.

    uv run build.py status      # 내 PC 에 뭐가 깔려 있나
    uv run build.py html        # 허브 페이지

여기 실린 도구는 전부 남이 만든 것이고 코드는 하나도 들어오지 않습니다 —
설치 «명령» 만 적혀 있습니다. 만든 곳은 [출처.md](출처.md) 에 전부 밝혔습니다.
